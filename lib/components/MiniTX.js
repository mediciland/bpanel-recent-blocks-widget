...
const MiniTX = ({
  ...
  const [goToTX, redirectToTX] = useState(false)
  useEffect(() => {
  ...
  let txType
  if (tx) {
    const uxtx = UXTX.fromRaw(tx.hex, 'hex', { constants: { DATE_FORMAT: 'MMMM Do YYYY, h:mm:ss a' }, json: tx })
  ...
  return <div
    className='card'
    style={{ margin: '5px', width: '50px', height: '50px', cursor: 'pointer', ...txColors[txType] }}
    onClick={clickOnTX}
  >
    {goToTX ? <Redirect to={`tx/${txid}`} /> : null}
